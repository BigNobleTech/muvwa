...
class _WithdrawalScreenState extends State<WithdrawalScreen> {
  ...
  Future<void> _submit() async {
    ...
    if (ok) {
      ...
      SnackbarHelper.showError(
        context,
        provider.error ?? 'wallet.withdraw_failed'.tr(),
      );
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    final methods = wallet.withdrawalMethods.isNotEmpty
        ...
        : _fallbackMethods;
    final selectedCode = methods.any((m) => m.code == _method)
        ? _method
        : methods.first.code;
    final selectedMethod = methods.firstWhere((m) => m.code == selectedCode);
              ...
              children: [
                ...
                TextFormField(
                  ...
                  validator: (v) {
                    ...
                    if (amt < _min) {
                      return 'wallet.min_withdraw'.tr(
                        namedArgs: {'amount': Helpers.currency(_min)},
                      );
                    }
                    if (amt > balance) {
            ...
            child: Column(
              ...
              children: [
                ...
                if (dt != null) ...[
                  const SizedBox(height: 2),
                  Text(
                    Helpers.formatDateTime(dt.toLocal()),
                    style: const TextStyle(
                      fontSize: 11,
                      color: AppColors.textSecondary,
                    ),
